...
void DetachDBinderCallbackStubFuzzTestTest(FuzzedDataProvider &provider)
{
    OHOS::IPCProcessSkeleton *current = OHOS::IPCProcessSkeleton::GetCurrent();
    if (current == nullptr) {
        return;
...
void QueryDBinderCallbackStubFuzzTestTest(FuzzedDataProvider &provider)
{
    OHOS::IPCProcessSkeleton *current = OHOS::IPCProcessSkeleton::GetCurrent();
    if (current == nullptr) {
        return;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::DetachDBinderCallbackStubFuzzTestTest(provider);
    OHOS::QueryDBinderCallbackStubFuzzTestTest(provider);
