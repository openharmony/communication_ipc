...
int DBinderTestServiceProxy::GetChildId(uint64_t &rep)
{
    ...
    DBINDER_LOGE(LABEL, "rep = %{public}" PRIu64 ", error = %{public}d", rep, error);
    return error;
}
...
int DBinderTestServiceProxy::TransProxyObjectRefCount(sptr<IRemoteObject> &transObject, int operation)
{
    ...
    if (error != ERR_NONE) {
        DBINDER_LOGE(LABEL, "fail to send proxy object");
        return ERR_INVALID_STATE;
    }
    return error;
}
...
int DBinderTestServiceProxy::TransStubObject(int data, sptr<IRemoteObject> &transObject, int &rep, int &stubRep)
{
    ...
    if (error != ERR_NONE) {
        DBINDER_LOGE(LABEL, "fail to send stub object");
        return ERR_INVALID_STATE;
    }
    ...
    if (proxy == nullptr) {
        DBINDER_LOGE(LABEL, "fail to get remote stub object");
        return ERR_INVALID_STATE;
    }
    ...
    if (error != ERR_NONE) {
        DBINDER_LOGE(LABEL, "fail to send data info");
        return ERR_INVALID_STATE;
    }
...
int DBinderTestServiceProxy::ProxyTransRawData(int length)
{
    ...
    DBINDER_LOGE(LABEL, "data length = %{public}d, wait time = %{public}d", length, waitTime);

    if (length <= 1) {
        DBINDER_LOGE(LABEL, "length should > 1, length is %{public}d", length);
        return ERR_INVALID_STATE;
    }
    unsigned char *buffer = new (std::nothrow) unsigned char[length];
    if (buffer == nullptr) {
        DBINDER_LOGE(LABEL, "new buffer failed of length = %{public}d", length);
        return ERR_INVALID_STATE;
    }
    buffer[0] = 'a';
    buffer[length - 1] = 'z';
    ...
    if (ret != ERR_NONE) {
        DBINDER_LOGE(LABEL, "fail to send request, ret = %{public}d", ret);
        return ret;
    }
    if (length != replyParcel.ReadInt32()) {
        DBINDER_LOGE(LABEL, "reply wrong length");
...
int DBinderTestServiceProxy::StubTransRawData(int length)
{
    ...
    if (replyParcel.ReadInt32() != length) {
        DBINDER_LOGE(LABEL, "reply false data");
        return ERR_INVALID_DATA;
    }

    const char *buffer = nullptr;
    if ((buffer = reinterpret_cast<const char *>(replyParcel.ReadRawData(length))) == nullptr) {
        DBINDER_LOGE(LABEL, "fail to read raw data, length = %{public}d", length);
        return ERR_INVALID_DATA;
    }
    if (buffer[0] != 'a' || buffer[length - 1] != 'z') {
        DBINDER_LOGE(LABEL, "received raw data is wrong, length = %{public}d", length);
        return ERR_INVALID_DATA;
    }

    if (replyParcel.ReadInt32() != length) {
        DBINDER_LOGE(LABEL, "fail to read length after raw data, length = %{public}d", length);
        return ERR_INVALID_DATA;
    }
...
int DBinderTestServiceProxy::FlushAsyncCommands(int count, int length)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (ret != ERR_NONE) {
            DBINDER_LOGE(LABEL, "fail to send request when count = %{public}d ret = %{public}d", i, ret);
            return ret;
        }
    }
...
int DBinderTestServiceProxy::PingService(std::u16string &serviceName)
{
    ...
    MessageOption option;
    MessageParcel dataParcel, replyParcel;
    DBINDER_LOGE(LABEL, "TestServiceProxy:PingService");
...
int DBinderTestServiceStub::OnRemoteRequest(uint32_t code, MessageParcel &data, MessageParcel &reply,
    MessageOption &option)
{
    DBINDER_LOGE(LABEL, "TestServiceStub::OnReceived, cmd = %{public}d", code);
...
int DBinderTestServiceStub::OnGetChildId(MessageParcel &data, MessageParcel &reply)
{
    ...
    if (!IPCSkeleton::SetCallingIdentity(token)) {
        DBINDER_LOGE(LABEL, "Set Calling Identity fail");
    }
...
int DBinderTestServiceStub::OnReceivedObject(MessageParcel &data, MessageParcel &reply)
{
    ...
    DBINDER_LOGI(LABEL, "%{public}s:TRANSOBJECT: reqData=%{public}d", __func__, reqData);
    int ret = proxy->SendRequest(REVERSEINT, dataParcel, replyParcel, option);
    int reqResult = replyParcel.ReadInt32();
    DBINDER_LOGI(LABEL, "%{public}s:TRANSOBJECT: result=%{public}d", __func__, reqResult);
...
int DBinderTestServiceStub::OnReceivedProxyObjectRefCount(MessageParcel &data, MessageParcel &reply)
{
    ...
    sptr<IRemoteObject> proxy = data.ReadRemoteObject();
    if (proxy == nullptr) {
        DBINDER_LOGE(LABEL, "fail to get proxy");
        return ERR_INVALID_STATE;
    }
...
int DBinderTestServiceStub::OnReceivedObjectTransAgain(MessageParcel &data, MessageParcel &reply)
{
    ...
    if (manager_ == nullptr) {
        DBINDER_LOGE(LABEL, "null manager_");
        return ERR_INVALID_STATE;
    }
    DBINDER_LOGI(LABEL, "%{public}s:OnReceivedObjectTransAgain-1: reqData=%{public}d", __func__, reqData);

    sptr<IRemoteObject> object = manager_->GetSystemAbility(RPC_TEST_SERVICE2);
    if (object == nullptr) {
        DBINDER_LOGE(LABEL, "null object of RPC_TEST_SERVICE2");
        return ERR_INVALID_STATE;
    }
    ...
    DBINDER_LOGI(LABEL, "%{public}s:OnReceivedObjectTransAgain-2: reqData=%{public}d", __func__, reqData);
    int ret = object->SendRequest(TRANS_RPC_OBJECT_TO_LOCAL, dataParcel, replyParcel, option);

    int reqResult = replyParcel.ReadInt32();
    DBINDER_LOGI(LABEL, "%{public}s:OnReceivedObjectTransAgain-3: result=%{public}d", __func__, reqResult);

    if (!reply.WriteInt32(reqResult)) {
        DBINDER_LOGE(LABEL, "fail to write parcel");
        return ERR_INVALID_STATE;
    }
    DBINDER_LOGI(LABEL, "%{public}s:OnReceivedObjectTransAgain-4: result=%{public}d", __func__, reqResult);
    if (!reply.WriteInt32(0)) {
        DBINDER_LOGE(LABEL, "fail to write parcel");
        return ERR_INVALID_STATE;
    }
    DBINDER_LOGI(LABEL, "%{public}s:OnReceivedObjectTransAgain-5: result=%{public}d", __func__, reqResult);
    return ret;
}
...
int DBinderTestServiceStub::OnReceivedStubObject(MessageParcel &data, MessageParcel &reply)
{
    ...
    if (!reply.WriteRemoteObject(this)) {
        DBINDER_LOGE(LABEL, "fail to write parcel stub");
        return ERR_INVALID_STATE;
    }
...
{
    DBINDER_LOGE(LABEL, "OnReceivedClearDecTimes");
...
int DBinderTestServiceStub::OnReceivedRawData(MessageParcel &data, MessageParcel &reply)
{
    ...
    if ((buffer = reinterpret_cast<const char *>(data.ReadRawData(length))) == nullptr) {
        ...
        DBINDER_LOGE(LABEL, "read raw data failed, length = %{public}d", length);
        return ERR_INVALID_DATA;
    }
    if (buffer[0] != 'a' || buffer[length - 1] != 'z') {
        if (!reply.WriteInt32(0)) {
        ...
        DBINDER_LOGE(LABEL, "buffer error, length = %{public}d", length);
        return ERR_INVALID_DATA;
    }
    if (data.ReadInt32() != length) {
        ...
        DBINDER_LOGE(LABEL, "read raw data after failed, length = %{public}d", length);
        return ERR_INVALID_DATA;
    }
    if (!reply.WriteInt32(length)) {
        DBINDER_LOGE(LABEL, "fail to write parcel");
...
void DBinderTestDeathRecipient::OnRemoteDied(const wptr<IRemoteObject> &remote)
{
    ...
    DBINDER_LOGE(LABEL, "recv death notification");
}
