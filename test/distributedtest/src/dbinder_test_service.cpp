...
static constexpr OHOS::HiviewDFX::HiLogLabel TEST_LOG_LABEL = { LOG_CORE, LOG_ID_TEST, "DBinderTest" };
...
{
    DBINDER_LOGI(TEST_LOG_LABEL, "DBinderTestService finish");
...
{
    DBINDER_LOGI(TEST_LOG_LABEL, "enter %{public}s", __func__);
    auto saMgr = SystemAbilityManagerClient::GetInstance().GetSystemAbilityManager();
    if (saMgr == nullptr) {
        DBINDER_LOGE(TEST_LOG_LABEL, "%{public}s:fail to instantiate", __func__);
        return -ENODEV;
    }
...
#endif
    DBINDER_LOGE(TEST_LOG_LABEL, "%{public}s: add TestService result=%{public}d", __func__, result);
...
int DBinderTestService::ReverseInt(int data, int &rep)
{
    DBINDER_LOGI(TEST_LOG_LABEL, "enter %{public}s", __func__);
    rep = Reverse(data);
    DBINDER_LOGI(TEST_LOG_LABEL, "%{public}s:read from client data = %{public}d", __func__, data);
    return ERR_NONE;
}
...
int DBinderTestService::Delay(int data, int &rep)
{
    ...
    while (i <= data) {
        sleep(1);
        DBINDER_LOGI(TEST_LOG_LABEL, "sleep loop : %{public}d", i);
        i++;
    }
    DBINDER_LOGE(TEST_LOG_LABEL, "%{public}s:read from client data = %{public}d", __func__, data);
    return ERR_NONE;
}
...
int DBinderTestService::PingService(std::u16string &serviceName)
{
    ...
    if (localServiceName.compare(serviceName) != 0) {
        DBINDER_LOGE(TEST_LOG_LABEL, "ServiceName is not equal");
        return -1;
    }
    return ERR_NONE;
}
...
int DBinderTestService::TransProxyObjectAgain(int data, sptr<IRemoteObject> &transObject, int operation, int &rep,
    int &withdrawRes)
{
    DBINDER_LOGI(TEST_LOG_LABEL, "enter");
    return 0;
}
...
{
    DBINDER_LOGI(TEST_LOG_LABEL, "DBinderTestService GetRemoteObject");
...
int DBinderTestService::GetRemoteDecTimes()
{
    ...
    DBINDER_LOGI(TEST_LOG_LABEL, "DBinderTestService GetDestructTimes");
...
void DBinderTestService::ClearRemoteDecTimes()
{
    ...
    DBINDER_LOGI(TEST_LOG_LABEL, "DBinderTestService ClearRemoteDecTimes");
    destructTimes_ = 0;
}
} // namespace OHOS
