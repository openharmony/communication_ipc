...
private:
    sptr<ISystemAbilityManager> manager_ = nullptr;
    char serverId_[DEVICEID_LENGTH + 1];
};

void DBinderTest::SetUpTestCase()
{
    DBINDER_LOGI(TEST_LOG_LABEL, "enter SetUpTestCase");
    StartDBinderServiceTestService();
}

void DBinderTest::TearDownTestCase()
{
    DBINDER_LOGI(TEST_LOG_LABEL, "enter TearDownTestCase");
...
bool DBinderTest::GetRemoteDeviceId()
{
    ...
    int ret = SendMessage(AGENT_NO::ONE, msg,
        ...
        if (memcpy_s(serverId_, DEVICEID_LENGTH, retId.c_str(), DEVICEID_LENGTH) != 0 || retLen != DEVICEID_LENGTH) {
            DBINDER_LOGE(TEST_LOG_LABEL, "fail to copy string");
            return false;
        }
