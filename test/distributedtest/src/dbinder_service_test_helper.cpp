...
static constexpr OHOS::HiviewDFX::HiLogLabel LABEL = { LOG_CORE, LOG_ID_TEST, "DBinderTestHelper" };
...
pid_t GetPidByName(std::string taskName)
{
    ...
    while ((ptr = readdir(dir)) != nullptr) {
        ...
        if (sprintf_s(filepath, sizeof(filepath), "/proc/%s/status", ptr->d_name) <= EOK) {
            DBINDER_LOGI(LABEL, "sprintf_s fail");
            closedir(dir);
            return INVALID_PID;
        }
        ...
        if (fp != nullptr) {
            ...
            if (sscanf_s(buf, "%*s %s", curTaskName, sizeof(curTaskName)) <= EOK) {
                DBINDER_LOGI(LABEL, "sscanf fail");
            }
...
int GetChildPids(std::vector<pid_t> &childPids)
{
    ...
    DBINDER_LOGI(LABEL, "current pid %{public}d", current);
    ...
    if (dir != nullptr) {
        while ((ptr = readdir(dir)) != nullptr) {
            ...
            DBINDER_LOGI(LABEL, "child pid %{public}d", child);
        }
        closedir(dir);
    }
...
pid_t StartExecutable(std::string name, std::string args)
{
    ...
    if (pid != INVALID_PID) {
        DBINDER_LOGI(LABEL, "test.service is already started, do nothing");
        return pid;
    }

    std::string cmd1 = "chmod +x /data/" + name;
    int res = system(cmd1.c_str());
    DBINDER_LOGI(LABEL, "%{public}s res = %d, errno = %{public}d", cmd1.c_str(), res, errno);

    std::string cmd2 = "/data/" + name + " " + args + "&";
    res = system(cmd2.c_str());
    DBINDER_LOGI(LABEL, "%{public}s res = %{public}d", cmd2.c_str(), res);
    ...
    while (pid == INVALID_PID && res < 10) { // 10:try-time to wait for exe start
        ...
        DBINDER_LOGI(LABEL, "StartExecutable pid = %{public}d && name = %{public}s", pid, name.c_str());
        usleep(100 * 1000); // 100:time-length 1000:time-unit
        res++;
    }

    DBINDER_LOGI(LABEL, "start %{public}s done", name.c_str());
...
{
    pid_t pid = GetPidByName(name);
    DBINDER_LOGI(LABEL, "StopExecutable %{public}s pid = %{public}d, prepare to kill it", name.c_str(), pid);

    if (pid != INVALID_PID) {
        DBINDER_LOGI(LABEL, "%{public}s pid = %{public}d, kill it", name.c_str(), pid);
        kill(pid, SIGKILL);
    }
}
...
int StartDBinderServiceSARegistry(void)
{
    ...
    if (registryPid != -1) {
        DBINDER_LOGI(LABEL, "SYSTEM_ABILITY_MANAGER_NAME Already Started pid=%{public}d", registryPid);
        return registryPid;
    }
    StartExecutable(SYSTEM_ABILITY_MANAGER_NAME);
    usleep(200 * 1000); // 100:200-length 1000:time-unit
    DBINDER_LOGI(LABEL, "Start SYSTEM_ABILITY_MANAGER_NAME done");
    return ERR_NONE;
}
...
void StartDBinderServiceTestService(void)
{
    ...
    DBINDER_LOGE(LABEL, "DBINDER_TEST_SERVICE_NAME pid : %{public}d", pid);

    pid = StartExecutable(DBINDER_TEST_SERVICE_NAME_SECOND);
    DBINDER_LOGE(LABEL, "DBINDER_TEST_SERVICE_NAME_SECOND pid : %{public}d", pid);
}

void StopDBinderServiceTestService(void)
{
    StopExecutable(DBINDER_TEST_SERVICE_NAME);
    DBINDER_LOGE(LABEL, "Stop DBINDER_TEST_SERVICE_NAME");

    StopExecutable(DBINDER_TEST_SERVICE_NAME_SECOND);
    DBINDER_LOGE(LABEL, "Stop DBINDER_TEST_SERVICE_NAME_SECOND");
}
