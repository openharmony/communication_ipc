 ...
 * @tc.desc: Verify the GetAndSaveDBinderData function when SaveDBinderData function DBINDER_CALLBACK_MALLOC_ERR
 * @tc.type: FUNC
 */
HWTEST_F(DBinderCallbackStubTest, GetAndSaveDBinderDataTest006, TestSize.Level1)
{
    sptr<DBinderCallbackStub> fakeStub = new (std::nothrow) DBinderCallbackStub(
        SERVICE_TEST, DEVICE_TEST, LOCALDEVICE_TEST, STUBINDEX_TEST, HANDLE_TEST, TOKENID_TEST);
    ASSERT_TRUE(fakeStub != nullptr);
 ...
 * @tc.desc: Verify the GetAndSaveDBinderData function when SaveDBinderData function ERR_NONE
 * @tc.type: FUNC
 */
HWTEST_F(DBinderCallbackStubTest, GetAndSaveDBinderDataTest007, TestSize.Level1)
{
    sptr<DBinderCallbackStub> fakeStub = new (std::nothrow) DBinderCallbackStub(
        SERVICE_TEST, DEVICE_TEST, LOCALDEVICE_TEST, STUBINDEX_TEST, HANDLE_TEST, TOKENID_TEST);
    ASSERT_TRUE(fakeStub != nullptr);
    ...
    EXPECT_CALL(mock, GetSAMgrObject()).WillOnce(Return(object));
    EXPECT_CALL(mock, GetSessionNameForPidUid(testing::_, testing::_)).WillOnce(Return(SESSION_TEST));
    ...
    fakeStub->AddDBinderCommAuth(pid, uid, sessionName);
    fakeStub->SaveDBinderData(sessionName);
    int32_t ret = fakeStub->GetAndSaveDBinderData(pid, uid);
    EXPECT_EQ(ret, ERR_NONE);
    std::fill(current->invokers_, current->invokers_ + IPCThreadSkeleton::INVOKER_MAX_COUNT, nullptr);
    delete invoker;
