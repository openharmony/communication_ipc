    ...
    std::shared_ptr<DBinderSessionObject> remoteSession)
{
    ...
    ZLOGI(LOG_LABEL, "serverName:%{public}s, stubIndex:%{public}" PRIu64
        ", peerTokenId = %{public}u, deviceId:%{public}s",
        serverName.c_str(), stubIndex, peerTokenId,
...
int DBinderDatabusInvoker::OnSendMessage(std::shared_ptr<DBinderSessionObject> sessionOfPeer)
{
    ...
    if (ret == 0) {
        ...
        ZLOGD(LOG_LABEL, "succ, seq:%{public}" PRIu64 ",  size:%{public}zd, channelId:%{public}" PRIu64,
            seqNumber_, size, session->GetChannelId());
    } else {
        ZLOGE(LOG_LABEL, "fail, ret:%{public}d, seq:%{public}" PRIu64 ", size:%{public}zd, channelId:%{public}" PRIu64,
...
int DBinderDatabusInvoker::OnSendRawData(std::shared_ptr<DBinderSessionObject> session, const void *data, size_t size)
{
    ...
    if (ret == 0) {
        ZLOGD(LOG_LABEL, "succ, seq:%{public}" PRIu64 ",  size:%{public}zu, channelId:%{public}" PRIu64,
            seqNumber_, size, dataBusSession->GetChannelId());
    } else {
        ZLOGE(LOG_LABEL, "fail, ret:%{public}d, seq:%{public}" PRIu64 ", size:%{public}zu, channelId:%{public}" PRIu64,
...
bool DBinderDatabusInvoker::OnDatabusSessionClientSideClosed(std::shared_ptr<Session> session)
{
    ...
    for (auto it = proxy.begin(); it != proxy.end(); ++it) {
        ...
        if (remoteObject != nullptr) {
            ...
            ZLOGE(LOG_LABEL, "cannot find proxy with descriptor: %{public}s", descStr8.c_str());
        }
    }
