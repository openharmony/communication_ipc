...
ani_value ani_MessageSequence::JS_writeStringArray(ani_env env, ani_callback_info info)
{
    size_t argc = 1;
    ani_value argv[ARGV_LENGTH_1] = {0};
    ani_value thisVar = nullptr;
    ...
    for (size_t i = 0; i < arrayLength; i++) {
        size_t bufferSize = 0;
        ani_value element = nullptr;
        ani_value checkElementResult = JS_checkWriteStringArrayElement(env, argv, i, bufferSize, element);
        if (checkElementResult == nullptr) {
            return checkElementResult;
        }
        char16_t stringValue[bufferSize + 1];
        size_t jsStringLength = 0;
        ani_get_value_string_utf16(env, element, stringValue, bufferSize + 1, &jsStringLength);
        if (jsStringLength != bufferSize) {
            ZLOGE(LOG_LABEL, "string length wrong");
            return napiErr.ThrowError(env, errorDesc::CHECK_PARAM_ERROR);
        }
...
ani_value ani_MessageSequence::JS_writeParcelable(ani_env env, ani_callback_info info)
{
    ...
    ani_is_exception_pending(env, &isPendingException);
    if (isPendingException) {
        ani_value lastException = nullptr;
        ZLOGE(LOG_LABEL, "call marshalling failed");
        ani_get_and_clear_last_exception(env, &lastException);
        napiSequence->nativeParcel_->RewindWrite(pos);
        return napiErr.ThrowError(env, errorDesc::WRITE_DATA_TO_MESSAGE_SEQUENCE_ERROR);
    }
...
ani_value ani_MessageSequence::JS_checkReadArrayArgs(ani_env env,
                                                       ani_callback_info info,
                                                       size_t &argc,
                                                       ani_value &thisVar,
                                                       ani_value* argv)
{
...
ani_value ani_MessageSequence::JS_readParcelableArrayCallJsFunc(ani_env env,
...
ani_value ani_MessageSequence::JS_create(ani_env env, ani_callback_info info)
{
    ...
    status = ani_get_named_property(env, global, "IPCSequenceConstructor_", &constructor);
    ani_ASSERT(env, status == ani_ok, "get message sequence constructor failed");
    ani_value jsMessageSequence;
    status = ani_new_instance(env, constructor, 0, nullptr, &jsMessageSequence);
    ani_ASSERT(env, status == ani_ok, "failed to  construct js MessageSequence");
    return jsMessageSequence;
}

ani_value ani_MessageSequence::JS_reclaim(ani_env env, ani_callback_info info)
{
    size_t argc = 0;
    ani_value thisVar = nullptr;
    ani_get_cb_info(env, info, &argc, nullptr, &thisVar, nullptr);
    ...
    ani_MessageSequence *napiSequence = nullptr;
    ani_remove_wrap(env, thisVar, (void **)&napiSequence);
    ani_ASSERT(env, napiSequence != nullptr, "napiSequence is null");
    delete napiSequence;
