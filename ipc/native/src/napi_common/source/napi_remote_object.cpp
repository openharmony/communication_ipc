...
int NAPIRemoteObject::OnJsRemoteRequest(CallbackParam *jsParam)
{
    ...
    napi_status sendRet = napi_send_event(env_, task, napi_eprio_immediate, "IPC_NAPIRemoteObject::OnJsRemoteRequest");
    if (sendRet != napi_ok) {
        ZLOGE(LOG_LABEL, "napi_send_event failed, ret:%{public}d", sendRet);
...
napi_value CreateJsProxyRemoteObject(napi_env env, const sptr<IRemoteObject> target)
{
    ...
    if (status != napi_o) {
        ZLOGE(LOG_LABEL, "new instance fail, status is %{public}d", static<int>(status));
    }
...
void StubExecuteSendRequest(napi_env env, SendRequestParam *param)
{
    ...
    napi_status sendRet = napi_send_event(env, task, napi_eprio_high, "IPC_StubExecuteSendRequest");
    if (sendRet != napi_ok) {
        ZLOGE(LOG_LABEL, "napi_send_event failed, ret:%{public}d", sendRet);
        if (param->callback) {
            napi_delete_reference(param->env, param->callback);
