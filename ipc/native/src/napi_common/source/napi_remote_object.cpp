    ...
    : IPCObjectStub(descriptor)
{
    ...
    if (jsThreadId_ == std::this_thread::get_id()) {
        ...
        if (uvRet != 0) {
            ...
            std::unique_lock<std::mutex> lock(param->lockInfo->mutex);
            if (!param->lockInfo->condition.wait_for(lock, std::chrono::seconds(WAIT_TIME),
...
int NAPIRemoteObject::OnJsRemoteRequest(CallbackParam *jsParam)
{
    ...
    if (uvRet != 0) {
        ...
        if (!jsParam->lockInfo->condition.wait_for(lock, std::chrono::seconds(WAIT_TIME),
            [&jsParam] { return jsParam->lockInfo->ready; })) {
