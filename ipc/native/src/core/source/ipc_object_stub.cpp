...
{
    ZLOGW(LABEL, "destroyed, desc:%{public}s, addr:%{public}s", Str16ToStr8(IPCProcessSkeleton::IpcConvertToString(descriptor_)).c_str(), Str16ToStr8(UIntToString(this)).c_str());
}
...
{
    ZLOGW(LABEL, "enter, desc:%{public}s", Str16ToStr8(descriptor_).c_str());
    IPCProcessSkeleton *current = IPCProcessSkeleton::GetCurrent();
    if (current == nullptr) {
