...
sptr<IRemoteObject> ProcessSkeleton::QueryObject(const std::u16string &descriptor, bool lockFlag)
{
    ...
    bool isValidObject = IsValidObject(remoteObject);
    if (isNullObject || !isValidObject || !remoteObject->AttemptIncStrong(this)) {
        ZLOGE(LOG_LABEL, "remoteObject is null or dead or AttemptIncStrong failed, "
            "isNullObject:%{public}d, isValidObject:%{public}d", isNullObject, isValidObject);
        return result;
    }
    result = remoteObject;
