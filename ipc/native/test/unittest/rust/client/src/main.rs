...
use ipc_rust::{
    ...
    RemoteObj, is_handling_transaction, Ylong, Runtime, IpcAsyncRuntime, IpcAsyncPool
};
...
mod async_test {
    ...
    fn test_check_services_async() {
        ...
        let _remote = Runtime::block_on(handle).expect("get iatest service failed");
    }

    #[test]
    fn test_sync_request() {
        let remote = Runtime::block_on(
            get_async_test_service::<dyn IATest<Ylong>>(IPC_TEST_SERVICE_ID)).expect("get iatest service failed");
    ...
    fn test_async_request() {
        let remote = Runtime::block_on(
            get_async_test_service::<dyn IATest<Ylong>>(IPC_TEST_SERVICE_ID)).expect("get iatest service failed");
        Runtime::block_on(remote.test_async_transaction(2019, 0)).expect("async ipc request failed");
    }
    ...
    fn test_parcel_interface_token() {
        let remote = Runtime::block_on(
            get_async_test_service::<dyn IATest<Ylong>>(IPC_TEST_SERVICE_ID)).expect("get iatest service failed");
        let token = InterfaceToken::new("Hello, Rust");
        let echo_token = Runtime::block_on(
            remote.echo_interface_token(&token)).expect("echo normal interface token failed");
        assert_eq!(token.get_token(), echo_token.get_token());

        let token = InterfaceToken::new("");
        let echo_token = Runtime::block_on(
            remote.echo_interface_token(&token)).expect("echo empty interface token failed");
        assert_eq!(token.get_token(), echo_token.get_token());
    }
...
fn test_transact_fd_by_async() {
    let object = get_service(IPC_TEST_SERVICE_ID).expect("get iatest service failed");
...
#[test]
fn test_transact_fd_by_async_002() {
    let object = get_service(IPC_TEST_SERVICE_ID).expect("get iatest service failed");
    let data = MsgParcel::new().expect("MsgParcel should success");

    let fd: FileDesc = Runtime::block_on(Ylong::spawn(
        move || object.send_request(ITestCode::CodeTransactFd as u32, &data, false),
        |reply| async move {
            let fd: FileDesc = reply.unwrap().read().unwrap();
            Ok(fd)
        }
    )).expect("get server fd failed");
    let mut info = String::new();
    let mut file = File::from(fd);
...
#[test]
fn test_transact_fd_by_async_003() {
    let object = get_service(IPC_TEST_SERVICE_ID).expect("get iatest service failed");
    let data = MsgParcel::new().expect("MsgParcel should success");

    let handle = Runtime::spawn_blocking(move || object.send_request(ITestCode::CodeTransactFd as u32, &data, false));
    let fd: FileDesc = Runtime::block_on(Box::pin(async move {
...
fn test_sync_to_async() {
    let remote_sync: RemoteObjRef<dyn ITest> = get_test_service();
    let remote_async = remote_sync.into_async::<Ylong>();
    let value = Runtime::block_on(
        remote_async.test_sync_transaction(2019, 0)).expect("sync ipc request failed");
    assert_eq!(value, 9102);
}

#[test]
fn test_async_to_sync() {
    let remote_async = Runtime::block_on(
        get_async_test_service::<dyn IATest<Ylong>>(IPC_TEST_SERVICE_ID)).expect("get iatest service failed");
    let remote_sync = remote_async.into_sync();
    let value = remote_sync.test_sync_transaction(2019, 0).expect("sync ipc request failed");
    assert_eq!(value, 9102);
}
