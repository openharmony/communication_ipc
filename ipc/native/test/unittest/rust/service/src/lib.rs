...
use ipc_rust::{
    ...
    String16, BoxFuture, IpcAsyncPool
};
...
define_remote_object!(
    ITest["test.ipc.ITestService"] {
        ...
        proxy: TestProxy,
        async: IATest, // Optional implementation of asynchronous traits
    }
);
...
/// Function between proxy and stub of ITestService
pub trait IATest<P>: IRemoteBroker {
    /// Test sync transaction
    fn test_sync_transaction(&self, value: i32, delay_time: i32) -> BoxFuture<'static, IpcResult<i32>>;
    /// Test async transaction
    fn test_async_transaction(&self, value: i32, delay_time: i32) -> BoxFuture<'static, IpcResult<()>>;
    /// Test ping service transaction
    fn test_ping_service(&self, service_name: &String16) -> BoxFuture<'static, IpcResult<()>>;
    /// Test file descriptor transaction
    fn test_transact_fd(&self) -> BoxFuture<'static, IpcResult<FileDesc>>;
    /// Test string transaction
    fn test_transact_string(&self, value: &str) -> BoxFuture<'static, IpcResult<i32>>;
    /// Test get foo service IPC object transaction
    fn test_get_foo_service(&self) -> BoxFuture<'static, IpcResult<RemoteObj>>;
    /// Test interface token transaction
    fn echo_interface_token(&self, token: &InterfaceToken) -> BoxFuture<'static, IpcResult<InterfaceToken>>;
    /// Test calling infomation transaction
    fn echo_calling_info(&self) -> BoxFuture<'static, IpcResult<(u64, u64, u64, u64)>>;
    /// Test get device id
    fn test_get_device_id(&self) -> BoxFuture<'static, IpcResult<(String, String)>>;
}

// Make RemoteStub<TestStub> object can call IATest function directly.
impl<P: IpcAsyncPool> IATest<P> for RemoteStub<TestStub> {
    fn test_sync_transaction(&self, value: i32, delay_time: i32) -> BoxFuture<'static, IpcResult<i32>> {
        let res = self.0.test_sync_transaction(value, delay_time);
        Box::pin(async move { res })
    }

    fn test_async_transaction(&self, value: i32, delay_time: i32) -> BoxFuture<'static, IpcResult<()>> {
        let res = self.0.test_async_transaction(value, delay_time);
        Box::pin(async move { res })
    }

    fn test_ping_service(&self, service_name: &String16) -> BoxFuture<'static, IpcResult<()>> {
        let res = self.0.test_ping_service(service_name);
        Box::pin(async move { res })
    }

    fn test_transact_fd(&self) -> BoxFuture<'static, IpcResult<FileDesc>> {
        let res = self.0.test_transact_fd();
        Box::pin(async move { res })
    }

    fn test_transact_string(&self, value: &str) -> BoxFuture<'static, IpcResult<i32>> {
        let res = self.0.test_transact_string(value);
        Box::pin(async move { res })
    }

    fn test_get_foo_service(&self) -> BoxFuture<'static, IpcResult<RemoteObj>> {
        let res = self.0.test_get_foo_service();
        Box::pin(async move { res })
    }

    fn echo_interface_token(&self, token: &InterfaceToken) -> BoxFuture<'static, IpcResult<InterfaceToken>> {
        let res = self.0.echo_interface_token(token);
        Box::pin(async move { res })
    }

    fn echo_calling_info(&self) -> BoxFuture<'static, IpcResult<(u64, u64, u64, u64)>> {
        let res = self.0.echo_calling_info();
        Box::pin(async move { res })
    }

    fn test_get_device_id(&self) -> BoxFuture<'static, IpcResult<(String, String)>> {
        let res = self.0.test_get_device_id();
        Box::pin(async move { res })
    }
}

impl<P: IpcAsyncPool> IATest<P> for TestProxy {
    fn test_sync_transaction(&self, value: i32, delay_time: i32) -> BoxFuture<'static, IpcResult<i32>> {
        let mut data = MsgParcel::new().expect("MsgParcel should success");
        data.write(&value).unwrap();
        data.write(&delay_time).unwrap();
        let remote = self.remote.clone();
        P::spawn(
            move || remote.send_request(ITestCode::CodeSyncTransaction as u32, &data, false),
            |reply| async move {
                let ret: i32 = reply.unwrap().read().expect("need reply i32");
                Ok(ret)
            }
        )
    }
    ...
    fn test_async_transaction(&self, value: i32, delay_time: i32) -> BoxFuture<'static, IpcResult<()>> {
        ...
        let remote = self.remote.clone();
        P::spawn(
            move || remote.send_request(ITestCode::CodeAsyncTransaction as u32, &data, true),
            |_| async move { Ok(()) }
        )
    }
    ...
    fn test_ping_service(&self, service_name: &String16) -> BoxFuture<'static, IpcResult<()>> {
        let mut data = MsgParcel::new().expect("MsgParcel should success");
        data.write(service_name).unwrap();
        let remote = self.remote.clone();
        P::spawn(
            move || remote.send_request(ITestCode::CodePingService as u32, &data, false),
            |_| async move { Ok(()) }
        )
    }
    ...
    fn test_transact_fd(&self) -> BoxFuture<'static, IpcResult<FileDesc>> {
        ...
        let remote = self.remote.clone();
        P::spawn(
            move || remote.send_request(ITestCode::CodeTransactFd as u32, &data, false),
            |reply| async move {
                let fd: FileDesc = reply.unwrap().read().unwrap();
                Ok(fd)
            }
        )
    }
    ...
    fn test_transact_string(&self, value: &str) -> BoxFuture<'static, IpcResult<i32>> {
        ...
        let remote = self.remote.clone();
        P::spawn(
            move || remote.send_request(ITestCode::CodeTransactString as u32, &data, false),
            |reply| async move {
                let len: i32 = reply.unwrap().read().unwrap();
                Ok(len)
            }
        )
    }
    ...
    fn test_get_foo_service(&self) -> BoxFuture<'static, IpcResult<RemoteObj>> {
        ...
        let remote = self.remote.clone();
        P::spawn(
            move || remote.send_request(ITestCode::CodeGetFooService as u32, &data, false),
            |reply| async move {
                let service: RemoteObj = reply.unwrap().read().unwrap();
                Ok(service)
            }
        )
    }
    ...
    fn echo_interface_token(&self, token: &InterfaceToken) -> BoxFuture<'static, IpcResult<InterfaceToken>> {
        ...
        let remote = self.remote.clone();
        P::spawn(
            move || remote.send_request(ITestCode::CodeInterfaceToekn as u32, &data, false),
            |reply| async move {
                let echo_value: InterfaceToken = reply.unwrap().read().expect("need reply token");
                Ok(echo_value)
            }
        )
    }
    ...
    fn echo_calling_info(&self) -> BoxFuture<'static, IpcResult<(u64, u64, u64, u64)>> {
        ...
        let remote = self.remote.clone();
        P::spawn(
            move || remote.send_request(ITestCode::CodeCallingInfo as u32, &data, false),
            |reply| async move {
                let val = reply.unwrap();
                let token_id: u64 = val.read().expect("need reply calling token id");
                let first_token_id: u64 = val.read().expect("need reply first calling token id");
                let pid: u64 = val.read().expect("need reply calling pid");
                let uid: u64 = val.read().expect("need reply calling uid");
                Ok((token_id, first_token_id, pid, uid))
            }
        )
    }
    ...
    fn test_get_device_id(&self) -> BoxFuture<'static, IpcResult<(String, String)>> {
        ...
        let remote = self.remote.clone();
        P::spawn(
            move || remote.send_request(ITestCode::CodeGetDeviceId as u32, &data, false),
            |reply| async move {
                let val = reply.unwrap();
                let local_device_id: String = val.read().expect("need reply calling local device id");
                let calling_device_id: String = val.read().expect("need reply first calling device id");
                Ok((local_device_id, calling_device_id))
            }
        )
    }
