...
DBinderServiceStubMock::DBinderServiceStubMock(const std::u16string &service, const std::string &device,
...
const std::u16string &DBinderServiceStubMock::GetServiceName()
{
    return serviceName_;
}

const std::string &DBinderServiceStubMock::GetDeviceID()
{
    return deviceID_;
}

// LCOV_EXCL_START
binder_uintptr_t DBinderServiceStubMock::GetBinderObject() const
{
    return binderObject_;
}
// LCOV_EXCL_STOP

uint32_t DBinderServiceStubMock::GetPeerPid()
{
    return peerPid_;
}

uint32_t DBinderServiceStubMock::GetPeerUid()
{
    return peerUid_;
}

void DBinderServiceStubMock::SetSeqNumber(uint32_t seqNum)
{
    seqNum_ = seqNum;
}

uint32_t DBinderServiceStubMock::GetSeqNumber()
{
    return seqNum_;
}

void DBinderServiceStubMock::SetNegoStatusAndTime(NegotiationStatus status, uint64_t time)
{
    negoStatus_ = status;
    negoTime_ = time;
}

void DBinderServiceStubMock::GetNegoStatusAndTime(NegotiationStatus &status, uint64_t &time)
{
    status = negoStatus_;
    time = negoTime_;
}

int32_t DBinderServiceStubMock::ProcessProto(uint32_t code, MessageParcel &data, MessageParcel &reply,
...
int32_t DBinderServiceStubMock::OnRemoteRequest(uint32_t code, MessageParcel &data, MessageParcel &reply,
    MessageOption &option)
{
...
int32_t DBinderServiceStubMock::ProcessDeathRecipient(MessageParcel &data)
{
...
int32_t DBinderServiceStubMock::AddDbinderDeathRecipient(MessageParcel &data)
{
...
int32_t DBinderServiceStubMock::RemoveDbinderDeathRecipient(MessageParcel &data)
{
    sptr<IRemoteObject> object = data.ReadRemoteObject();
    if (object == nullptr) {
        DBINDER_LOGE(LOG_LABEL, "received proxy is null");
...
bool DBinderServiceStubMock::Marshalling(Parcel &parcel) const
{
...
bool DBinderServiceStubMock::Marshalling(Parcel &parcel, const sptr<IRemoteObject> &object)
{
    DBINDER_LOGD(LOG_LABEL, "enter");
    auto serviceStub = reinterpret_cast<DBinderServiceStubMock *>(object.GetRefPtr());
...
int DBinderServiceStubMock::SaveDBinderData(const std::string &localBusName)
{
    sptr<DBinderService> dBinderService = DBinderService::GetInstance();
    if (dBinderService == nullptr) {
        DBINDER_LOGE(LOG_LABEL, "DBinderService is nullptr");
...
// LCOV_EXCL_START
bool DBinderServiceStubMock::CheckSessionObjectValidity()
{
    sptr<DBinderService> dBinderService = DBinderService::GetInstance();
    if (dBinderService == nullptr) {
        DBINDER_LOGE(LOG_LABEL, "DBinderService is nullptr");
...
int DBinderServiceStubMock::GetAndSaveDBinderData(pid_t pid, uid_t uid)
{
    if (isInited_) {
...
int DBinderServiceStubMock::DBinderClearServiceState(uint32_t code, MessageParcel &data, MessageParcel &reply,
    MessageOption &option)
{
