...
class DBinderServiceStubMock : public IPCObjectStub {
public:
    explicit DBinderServiceStubMock(const std::u16string &serviceName, const std::string &deviceID,
        binder_uintptr_t binderObject, uint32_t pid = 0, uint32_t uid = 0);
    ~DBinderServiceStubMock();

    /**
     * @brief Serialize a specified DBinderServiceStubMock object.
