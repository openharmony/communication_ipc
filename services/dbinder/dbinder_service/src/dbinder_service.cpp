...
sptr<DBinderServiceStub> DBinderService::FindDBinderStub(const std::u16string &service, const std::string &device)
{
    ...
    auto it = std::find_if(DBinderStubRegisted_.begin(), DBinderStubRegisted_.end(), checkStub);
    if (it == DBinderStubRegisted_.end()) {
        DBINDER_LOGW(LOG_LABEL, "not found, %{public}s %{public}s", Str16ToStr8(service).c_str(),
            DBinderService::ConvertToSecureDeviceID(device).c_str());
        return nullptr;
    }
    DBINDER_LOGD(LOG_LABEL, "found, %{public}s %{public}s", Str16ToStr8(service).c_str(),
        DBinderService::ConvertToSecureDeviceID(device).c_str());
    return (*it);
}
...
bool DBinderService::DeleteDBinderStub(const std::u16string &service, const std::string &device)
{
    ...
    DBINDER_LOGI(LOG_LABEL, "succ, %{public}s %{public}s", Str16ToStr8(service).c_str(),
        DBinderService::ConvertToSecureDeviceID(device).c_str());
    return true;
}
    ...
    binder_uintptr_t binderObject)
{
    ...
    auto it = std::find_if(DBinderStubRegisted_.begin(), DBinderStubRegisted_.end(), checkStub);
    if (it != DBinderStubRegisted_.end()) {
        DBINDER_LOGD(LOG_LABEL, "found, %{public}s %{public}s", serviceStr8.c_str(),
            DBinderService::ConvertToSecureDeviceID(device).c_str());
        return (*it);
    }
    ...
    DBINDER_LOGD(LOG_LABEL, "create, %{public}s %{public}s", serviceStr8.c_str(),
        DBinderService::ConvertToSecureDeviceID(device).c_str());
    ...
    uint32_t uid)
{
    ...
    if (remoteListener == nullptr) {
        DBINDER_LOGE(LOG_LABEL, "remoteListener is null, service:");
        return false;
    }
...
bool DBinderService::OnRemoteInvokerMessage(const struct DHandleEntryTxRx *message)
{
    ...
    if (memcpy_s(replyMessage.get(), sizeof(DHandleEntryTxRx), message, sizeof(DHandleEntryTxRx)) != 0) {
        DBINDER_LOGE(LOG_LABEL, "memory DHandleEntryTxRx fail");
        return false;
    }
    ...
    std::string &remoteDeviceId, int pid, int uid, uint32_t tokenId)
{
    ...
    if (memcpy_s(replyMessage->serviceName, SERVICENAME_LENGTH, serverSessionName.data(),
        ...
        DBINDER_LOGE(LOG_LABEL, "memory serviceName fail, handle:%{public}d", proxy->GetHandle());
        return SESSION_NAME_INVALID;
    }
...
{
    DBINDER_LOGI(LOG_LABEL, "invoke remote stubIndex:%{public}d, type:%{public}u, seq:%{public}u",
